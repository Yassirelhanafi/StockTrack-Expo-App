...
export default function ProductsScreen() {
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: (data, productId) => {
            ...
            queryClient.invalidateQueries({ queryKey: ['localProducts'] }); // Refresh the list
             // Also try to invalidate Firebase data if it exists
    ...
    const handleDelete = (id: string, name: string) => {
        Alert.alert(
            "Confirm Deletion",
            `Are you sure you want to remove "${name}" (ID: ${id}) from local storage?`,
            [
    ...
    if (error) {
        return (
            <View style={styles.centered}>
                <Ionicons name="alert-circle-outline" size={40} color="red" style={styles.errorIcon}/>
    ...
    if (!products || products.length === 0) {
        return (
            <View style={styles.centered}>
                <Ionicons name="file-tray-outline" size={50} color="#9ca3af" style={styles.emptyIcon} />
...
const styles = StyleSheet.create({
    ...
    quantityBadge: {
        ...
        borderRadius: 15, // Pill-shaped
        minWidth: 45, // Min width for badge
        justifyContent: 'center',
        alignItems: 'center',
        marginRight: 10, // Space between badge and delete button
    },
    lowStockBadge: {
    ...
    quantityText: {
        fontWeight: 'bold',
        fontSize: 14,
         color: '#374151', // Dark gray text for quantity (works on light backgrounds)
    },
    deleteButton: {
        padding: 8, // Make hit area larger
         borderRadius: 20, // Circular background on press (optional)
        // backgroundColor: '#fee2e2', // Optional light red background on hover/press
    },
    loadingText: {
        marginTop: 10,
        fontSize: 16,
